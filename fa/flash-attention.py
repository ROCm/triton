...
def get_cdna_autotune_configs():
    return [
        # triton.Config({'BLOCK_M': 64, 'BLOCK_N': 64, 'waves_per_eu': 0, 'PRE_LOAD_V': False, 'GRID_CU_MULTIP': 2},
        #               num_stages=2, num_warps=4),
        triton.Config({'BLOCK_M': 256, 'BLOCK_N': 64, 'waves_per_eu': 2, 'PRE_LOAD_V': False, 'GRID_CU_MULTIP': 2},
        ...
        if continue_condition:
            ...
            if continue_condition:
                ...
                if (masked_blocks > 0):
                    ...
                    k_ptrs = k_offset + offs_d[:, None] * stride_kk + offs_n[None, :] * stride_kn + n_full_blocks * BLOCK_N * stride_kn
                    #v_ptrs = n_full_blocks * BLOCK_N * stride_vk
                    v_ptrs = v_offset + offs_n[:, None] * stride_vk + offs_d[None, :] * stride_vn + n_full_blocks * BLOCK_N * stride_vk
                    if USE_BIAS:
