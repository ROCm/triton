...
def _attn_fwd_inner(
    ...
    block_min,
    block_max,
    dtypeq_fpflag: tl.constexpr, dtypekv_fpflag: tl.constexpr,
    OFFS_M: tl.constexpr,
    OFFS_N: tl.constexpr,
    ...
    for _ in range(block_min, block_max, BLOCK_N):
        ...
        if dtypeq_fpflag == 82:
            qk += tl.dot_scaled(q, q_scale, 'e5m2', k, k_scale, 'e2m1')
        if dtypeq_fpflag == 83:
            qk += tl.dot_scaled(q, q_scale, 'e4m3', k, k_scale, 'e2m1')
...
def _attn_fwd(
    ...
    offs_n_packed = tl.arange(0, BLOCK_N // 2)
...
def attn_fwd(q, k, v, q_scale, k_scale, v_scale, config, args):
    ...
    dtypeq_fpflag = getfpflag(args.q_type)
    dtypekv_fpflag = getfpflag(args.kv_type)
...
def test_mha(config, args):
    ...
    def create_scale(b: int, s: int, h: int, d: int, scale_dim: int):
        size=[b, s, h, d]
        size[scale_dim] //= 32
        scale = MXScaleTensor(size=tuple(size)).random(high=32)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--q-type", choices=["float8e5m2", "float8e4nv"],
                      default="float8e5m2",
                      help="data type for K and V")
  parser.add_argument("--kv-type", choices=["float8e5m2", "float4"],
                      default="float4",
                      help="data type for K and V")
