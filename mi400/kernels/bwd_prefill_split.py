...
@triton.jit
def _bwd_preprocess(O, DO,  # noqa: E741
                    Delta, stride_ob, stride_oh, stride_om, stride_ok, stride_deltab, stride_deltah, stride_deltam,
                    stride_descale_do_z, cu_seqlens_q, max_seqlen_q, Descale_do, BLOCK_M: tl.constexpr,
                    HEAD_DIM: tl.constexpr, ACTUAL_HEAD_DIM: tl.constexpr, IS_VARLEN: tl.constexpr,
                    IS_FP8: tl.constexpr):
    ...
    for blk_idx in range(num_steps):
        ...
        if ENABLE_DROPOUT:
            ...
            if IS_FP8:
                ...
                dv += (tl.dot(
                    (pT_dropout * scale_p_dropout).to(do.type.element_ty), do) * descale_p_dropout * descale_do)
            else:
...
def _bwd_kernel_dkdv_causal(
    ...
    adj_k = bid * stride_kb + hkid * stride_kh + k_start * stride_kn + offs_n[:, None] * stride_kn + offs_k[
        None, :] * stride_kk
    adj_v = bid * stride_vb + hkid * stride_vh + k_start * stride_vn + offs_n[:, None] * stride_vn + offs_k[
        None, :] * stride_vk
    ...
    for hqid in range(hkid * GROUP_SIZE, hkid * GROUP_SIZE + GROUP_SIZE):
        ...
        if DEBUG_TRITON:
            print(f"unMasked: start_n: {start_n}, start_m: {start_m}, end_m: {end_m}, num_steps: {num_steps}"
                  )  # noqa: E701
...
def _bwd_kernel_dq_causal(
    ...
    if start_m + BLOCK_M < delta_qk:
        if DEBUG_TRITON:
            print(f"start_m + BLOCK_M = {start_m} + {BLOCK_M} = {start_m + BLOCK_M} < delta_qk of {delta_qk}"
                  )  # noqa: E701
        return
    ...
    for hqid in range(hkid * GROUP_SIZE, hkid * GROUP_SIZE + GROUP_SIZE):
        ...
        if DEBUG_TRITON:
            print(
                f"Masked: start_m: {start_m}, start_n: {start_n}, end_n: {end_n}, num_steps: {num_steps}")  # noqa: E701
        dq = _bwd_dq_inner(
            dq,
            q,
            K,
        ...
        if DEBUG_TRITON:
            print(f"unMasked: start_m: {start_m}, start_n: {start_n}, end_n: {end_n}, num_steps: {num_steps}"
                  )  # noqa: E701
        dq = _bwd_dq_inner(
            dq,
            q,
...
def attention_prefill_backward_triton_split_impl(
    ...
    padded_d_model = max(padded_d_model,
                         32)  # NOTE: the causal path expects a min of 32. It will cause a compiler assert.
    ...
    _bwd_preprocess[pre_grid](o, do, delta, stride_ob, stride_oh, stride_om, stride_ok, stride_deltab, stride_deltah,
                              stride_deltam, stride_descale_do_z, cu_seqlens_q, max_seqlen_q, descale_do,
                              BLOCK_M=PRE_BLOCK, HEAD_DIM=HEAD_DIM, ACTUAL_HEAD_DIM=ACTUAL_HEAD_DIM,
                              IS_VARLEN=IS_VARLEN, IS_FP8=IS_FP8)
    ...
    stride_dropoutb, stride_dropouth, stride_dropoutm, stride_dropoutn = \
        (0, 0 , 0 , 0)
    if use_dropout:
        dropout_mask = torch.zeros((batch, nheads_q, max_seqlen_q, max_seqlen_k), device=q.device, dtype=torch.float32)

        if DROPOUT_USE_PYTORCH:
            if not IS_VARLEN:
                dropout_mask = create_dropout_mask(dropout_p, (batch, nheads_q, max_seqlen_q, max_seqlen_k),
                                                   seed=philox_seed)
            else:
                dropout_mask = create_dropout_mask_varlen(dropout_p, batch, nheads_q, cu_seqlens_q, cu_seqlens_k,
                                                          philox_seed)
        stride_dropoutb, stride_dropouth, stride_dropoutm, stride_dropoutn = \
            dropout_mask.stride()
    ...
    if causal:
        if DEBUG_TRITON:
            print(f"_bwd_kernel_dkdv: grid = {grid_dkdv}, block_size = ({BLOCK_M1, BLOCK_N1})", )  # noqa: E701
