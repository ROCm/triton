...
DEBUG_TRITON_DETAIL = os.environ.get('DEBUG_TRITON_DETAIL',
                                     '0').lower() in ('1', 'true', 'yes') and USE_TRITON_INTERPRET
...
class MetaData():
    ...
    def check_args(self, q, k, v, o):
        ...
        batch, nheads_q, nheads_k, head_size, _, _ = get_shape_from_layout(q, k, self.layout, self.cu_seqlens_q,
                                                                           self.cu_seqlens_k, self.max_seqlens_q,
                                                                           self.max_seqlens_k)
        if self.varlen:
            assert q.dim() == 3
...
def generate_varlen_tensor(batch_size: int, total_seqlen: int, num_heads: int, head_size: int,
                           equal_seqlens: bool = False, device: str = "cuda", dtype: torch.dtype = torch.float32,
                           DEBUG_INPUT: bool = False) -> tuple[torch.Tensor, torch.Tensor, int]:
    # get seqlens
    if equal_seqlens:
        seqlens = torch.full((batch_size, ), total_seqlen // batch_size, dtype=torch.int32, device=device)
        seqlens[-1] += total_seqlen % batch_size
    else:
        seqlens = random_seqlens_composition(total_seqlen, batch_size).to(device=device)

    # create cumulative sequence lengths
    cu_seqlens = torch.cat([torch.tensor([0], dtype=torch.int32, device=device),
                            seqlens.cumsum(dim=0)]).to(torch.int32).to(device=device)
    ...
    if DEBUG_INPUT:
        ...
        for i in range(batch_size):
            ...
            x[start:end, :, :] = (torch.arange(length, dtype=dtype,
                                               device=device).view(length, 1, 1).expand(length, num_heads, head_size))
    else:
...
def varlen_input_helper(BATCH, HQ, HK, TOTAL_SEQLENS_Q, TOTAL_SEQLENS_K, D_HEAD, dtype, device="cuda",
                        equal_seqlens=False, DEBUG_INPUT=False):
    torch.manual_seed(20)
    q, cu_seqlens_q, _ = generate_varlen_tensor(BATCH, TOTAL_SEQLENS_Q, HQ, D_HEAD, dtype=dtype, device=device,
                                                equal_seqlens=equal_seqlens, DEBUG_INPUT=DEBUG_INPUT)
    k, cu_seqlens_k, _ = generate_varlen_tensor(BATCH, TOTAL_SEQLENS_K, HK, D_HEAD, dtype=dtype, device=device,
                                                equal_seqlens=equal_seqlens, DEBUG_INPUT=DEBUG_INPUT)
    v, _, _ = generate_varlen_tensor(BATCH, TOTAL_SEQLENS_K, HK, D_HEAD, dtype=dtype, device=device,
                                     equal_seqlens=equal_seqlens, DEBUG_INPUT=DEBUG_INPUT)
    sm_scale = D_HEAD**-0.5
    ...
    if DEBUG_INPUT:
        if layout == "bhsd":
            q = torch.arange(N_CTX_Q, dtype=dtype,
                             device=device).view(1, 1, N_CTX_Q,
                                                 1).expand(*q_tensor_shape).contiguous().requires_grad_()
            k = torch.arange(N_CTX_K, dtype=dtype,
                             device=device).view(1, 1, N_CTX_K,
                                                 1).expand(*k_tensor_shape).contiguous().requires_grad_()
            v = torch.arange(N_CTX_K, dtype=dtype,
                             device=device).view(1, 1, N_CTX_K,
                                                 1).expand(*k_tensor_shape).contiguous().requires_grad_()
        elif layout == "bshd":
            q = torch.arange(N_CTX_Q, dtype=dtype,
                             device=device).view(1, N_CTX_Q, 1,
                                                 1).expand(*q_tensor_shape).contiguous().requires_grad_()
            k = torch.arange(N_CTX_K, dtype=dtype,
                             device=device).view(1, N_CTX_K, 1,
                                                 1).expand(*k_tensor_shape).contiguous().requires_grad_()
            v = torch.arange(N_CTX_K, dtype=dtype,
                             device=device).view(1, N_CTX_K, 1,
                                                 1).expand(*k_tensor_shape).contiguous().requires_grad_()
...
def input_helper(BATCH, HQ, HK, N_CTX_Q, N_CTX_K, D_HEAD, dtype, layout, device="cuda", DEBUG_INPUT=False):
    if layout == "thd":
        q, k, v, metadata = varlen_input_helper(BATCH, HQ, HK, N_CTX_Q, N_CTX_K, D_HEAD, dtype, device=device,
                                                DEBUG_INPUT=DEBUG_INPUT)
    else:
        q, k, v, metadata = nonvarlen_input_helper(BATCH, HQ, HK, N_CTX_Q, N_CTX_K, D_HEAD, dtype, layout,
                                                   device=device, DEBUG_INPUT=DEBUG_INPUT)
...
def decast_fp8(x_fp8: torch.Tensor, descale_factor: torch.Tensor, original_dtype: torch.dtype, layout: str,
...
def cast_to_fp8(x: torch.Tensor, fp8_dtype: torch.dtype, layout: str, clamp_val: float = 1e-9,
                cu_seqlens=None) -> tuple[torch.Tensor, torch.Tensor]:
    if layout in ("bshd", "bhsd"):
...
def create_dropout_mask(dropout_p, shape, seed):
    device = "cuda"
    rand_vals = torch.rand(shape, generator=torch.Generator(device=device).manual_seed(seed), device=device,
                           dtype=torch.float32)
...
def create_dropout_mask_varlen(dropout_p, batch, nheads_q, cu_seqlens_q, cu_seqlens_k, philox_seed):
    ...
    for b in range(batch):
        qlen = qlens[b]
        klen = klens[b]
        rand_vals = torch.rand((nheads_q, qlen, klen),
                               generator=torch.Generator(device=device).manual_seed(philox_seed), device=device,
                               dtype=torch.float32)
