...
@gluon.jit
def issue_loads(producer, a_desc, b_desc, a_buffer, b_buffer, BLOCK_K: ttgl.constexpr, NUM_BUFFERS: ttgl.constexpr,
                TRANSPOSE_B: ttgl.constexpr):
    ttgl.amd.gfx1250.tdm.async_load(a_desc, [0, producer * BLOCK_K],  #
                                    a_buffer.index(producer % NUM_BUFFERS))
    if not TRANSPOSE_B:
        ttgl.amd.gfx1250.tdm.async_load(b_desc, [producer * BLOCK_K, 0],  #
                                        b_buffer.index(producer % NUM_BUFFERS))
    else:
        ttgl.amd.gfx1250.tdm.async_load(b_desc, [0, producer * BLOCK_K],  #
                                        b_buffer.index(producer % NUM_BUFFERS))
    producer += 1
    return producer
...
def persistent_gemm_tdm_pipelined_kernel(a_ptr, b_ptr, c_ptr,  #
    ...
    for tile_idx in range(scheduler.get_num_tiles()):
        ...
        for _ in ttgl.static_range(NUM_BUFFERS - 1):
            producer = issue_loads(producer, a_desc, b_desc, a_buffer, b_buffer, BLOCK_K, NUM_BUFFERS, TRANSPOSE_B)
...
def test_runtime_gemm_tdm_pipelined(BLOCK_M, BLOCK_N, BLOCK_K, NUM_BUFFERS, TRANSPOSE_B, PERSISTENT, M, N, K,
                                    num_warps):
...
if __name__ == "__main__":
    ...
    PERSISTENT = True
    print(
        f"({M=}, {N=}, {K=}), ({BLOCK_M=}, {BLOCK_N=}, {BLOCK_K=}), {NUM_BUFFERS=}, {TRANSPOSE_B=}, {PERSISTENT=}, {NUM_WARPS=}"
    )
    test_runtime_gemm_tdm_pipelined(BLOCK_M, BLOCK_N, BLOCK_K, NUM_BUFFERS, TRANSPOSE_B, PERSISTENT, M, N, K, NUM_WARPS)
    NUM_WARPS = 8
    print(
        f"({M=}, {N=}, {K=}), ({BLOCK_M=}, {BLOCK_N=}, {BLOCK_K=}), {NUM_BUFFERS=}, {TRANSPOSE_B=}, {PERSISTENT=}, {NUM_WARPS=}"
    )
