...
@gluon.jit
def mxgemm_tdm_2_stages_pipeline_kernel(a_ptr, b_ptr, c_ptr, a_scale, b_scale, M, N, K, stride_am, stride_ak, stride_bk,
    ...
    SHARED_LAYOUT_A: gl.constexpr = gl.PaddedSharedLayout.with_identity_for([[32, 4]], [BLOCK_M, BLOCK_K_PACKED_A],
                                                                            [1, 0])
    SHARED_LAYOUT_B: gl.constexpr = gl.PaddedSharedLayout.with_identity_for([[32, 4]], [BLOCK_K_PACKED_B, BLOCK_N],
                                                                            [1, 0])

    SHARED_LAYOUT_A_SCALE: gl.constexpr = gl.PaddedSharedLayout.with_identity_for([[8, 4]], [BLOCK_M, BLOCK_K_SCALE],
                                                                                  [1, 0])
    SHARED_LAYOUT_B_SCALE: gl.constexpr = gl.PaddedSharedLayout.with_identity_for([[8, 4]], [BLOCK_N, BLOCK_K_SCALE],
                                                                                  [1, 0])
    ...
    a_buffer = gl.allocate_shared_memory(a_desc.dtype, shape=[2] + a_desc.block_shape, layout=a_desc.layout)
    ...
    b_desc = gl.amd.gfx1250.tdm.make_tensor_descriptor(base=b_ptr + pid_n * BLOCK_N * stride_bn,
                                                       ...
                                                       block_shape=(BLOCK_K_PACKED_B, BLOCK_N), layout=SHARED_LAYOUT_B)
    b_buffer = gl.allocate_shared_memory(b_desc.dtype, shape=[2] + b_desc.block_shape, layout=b_desc.layout)
    ...
    a_scale_buffer = gl.allocate_shared_memory(a_scale_desc.dtype, shape=[2] + a_scale_desc.block_shape,
    ...
    b_scale_buffer = gl.allocate_shared_memory(b_scale_desc.dtype, shape=[2] + b_scale_desc.block_shape,
                                               layout=b_scale_desc.layout)

    # prologue
    gl.amd.gfx1250.tdm.async_load(a_desc, [0, 0], a_buffer.index(0))
    gl.amd.gfx1250.tdm.async_load(b_desc, [0, 0], b_buffer.index(0))
    gl.amd.gfx1250.tdm.async_load(a_scale_desc, [0, 0], a_scale_buffer.index(0))
    gl.amd.gfx1250.tdm.async_load(b_scale_desc, [0, 0], b_scale_buffer.index(0))

    accumulator = gl.zeros((BLOCK_M, BLOCK_N), dtype=gl.float32, layout=WMMA_LAYOUT)
    num_iterations = gl.cdiv(K, BLOCK_K)
    for k in range(1, num_iterations):
        gl.amd.gfx1250.tdm.async_wait(0)

        gl.amd.gfx1250.tdm.async_load(a_desc, [0, k * BLOCK_K_PACKED_A], a_buffer.index(k % 2))
        gl.amd.gfx1250.tdm.async_load(b_desc, [k * BLOCK_K_PACKED_B, 0], b_buffer.index(k % 2))
        gl.amd.gfx1250.tdm.async_load(a_scale_desc, [0, k * BLOCK_K_SCALE], a_scale_buffer.index(k % 2))
        gl.amd.gfx1250.tdm.async_load(b_scale_desc, [0, k * BLOCK_K_SCALE], b_scale_buffer.index(k % 2))

        a = a_buffer.index((k + 1) % 2).load(layout=DOT_LAYOUT_A)
        b = b_buffer.index((k + 1) % 2).load(layout=DOT_LAYOUT_B)
        scale_a = a_scale_buffer.index((k + 1) % 2).load(layout=A_SCALE_LINEAR_LAYOUT)
        scale_b = b_scale_buffer.index((k + 1) % 2).load(layout=B_SCALE_LINEAR_LAYOUT)

        accumulator = gl.amd.gfx1250.wmma_scaled(a, scale_a, DTYPE_A, b, scale_b, DTYPE_B, accumulator)

    # epilogue
    gl.amd.gfx1250.tdm.async_wait(0)
    a = a_buffer.index((num_iterations + 1) % 2).load(layout=DOT_LAYOUT_A)
    b = b_buffer.index((num_iterations + 1) % 2).load(layout=DOT_LAYOUT_B)
    scale_a = a_scale_buffer.index((num_iterations + 1) % 2).load(layout=A_SCALE_LINEAR_LAYOUT)
    scale_b = b_scale_buffer.index((num_iterations + 1) % 2).load(layout=B_SCALE_LINEAR_LAYOUT)
    accumulator = gl.amd.gfx1250.wmma_scaled(a, scale_a, DTYPE_A, b, scale_b, DTYPE_B, accumulator)
...
def run(config):
    ...
    mxgemm_tdm_2_stages_pipeline_kernel[grid](a_d, b_d, c_d, a_scale_d, b_scale_d, M, N, K, stride_am, stride_ak,
                                              stride_bk, stride_bn, stride_cm, stride_cn, stride_scale,
                                              dtype_converter[dtype_a], dtype_converter[dtype_b], scale_block,
                                              blockSizeM, blockSizeN, blockSizeK, group_size_m, num_warps=numWarps,
                                              num_ctas=numCtas)
...
if __name__ == '__main__':
    for dtypeA, dtypeB in (("float8_e5m2", "float4"), ("float8_e4m3", "float8_e5m2"), ("float4", "float4")):
        config = {
            ...
            1, "SCALE_BLOCK": 32, "DTYPE_A": dtypeA, "DTYPE_B": dtypeB
        }
        run(config)
