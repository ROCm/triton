...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "hip:gfx950", "ttg.threads-per-warp" = 64 : i32} {
  ...
  tt.func public @simple_tdm_waitcnt(%memDesc: !ttg.memdesc<128x16xf16, #shared, #smem, mutable>, %tensorDesc: !tt.tensordesc<tensor<128x16xf16>>, %mask: i1
    ...
    %c0_i32 = arith.constant 0 : i32
    %wave_id = amdgpu.get_wave_id : i32

    // Each async_tdm_copy only emits a single instruction (-> counts 1)
    %1 = amdgpu.async_tdm_copy_global_to_local %tensorDesc %c0_i32, %c0_i32 %memDesc, %mask, %wave_id : <tensor<128x16xf16>> -> <128x16xf16, #shared, #smem, mutable>
    %2 = amdgpu.async_tdm_copy_global_to_local %tensorDesc %c0_i32, %c0_i32 %memDesc, %mask, %wave_id : <tensor<128x16xf16>> -> <128x16xf16, #shared, #smem, mutable>

    // Do not wait on the second tdm => waitcnt 1
    // CHECK: amdgpu.async_tdm_store_wait {{.*}} {num = 1
    %w1 = amdgpu.async_tdm_store_wait %1 {num = 0 : i32}
    // No async_copies in between => waitcnt 0
    // CHECK: amdgpu.async_tdm_store_wait {{.*}} {num = 0
    %w2 = amdgpu.async_tdm_store_wait %2 {num = 0 : i32}
    tt.return
  }
}
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "hip:gfx950", "ttg.threads-per-warp" = 64 : i32} {
  ...
  tt.func public @mix_async_copy_and_async_tdm_copy(%memDesc: !ttg.memdesc<128x16xf16, #shared, #smem, mutable>, %tensorDesc: !tt.tensordesc<tensor<128x16xf16>>, %mask: i1, %ptr: tensor<128x16x!tt.ptr<f16>, #blocked>
  ) {
    %c0_i32 = arith.constant 0 : i32
    %wave_id = amdgpu.get_wave_id : i32
    ...
    %3 = amdgpu.async_tdm_copy_global_to_local %tensorDesc %c0_i32, %c0_i32 %memDesc, %mask, %wave_id : <tensor<128x16xf16>> -> <128x16xf16, #shared, #smem, mutable>
    ...
    // CHECK: amdgpu.async_tdm_store_wait {{.*}} {num = 1
    %tw1 = amdgpu.async_tdm_store_wait %1 {num = 0 : i32}

    // CHECK: ttg.async_wait {{.*}} {num = 2
    %cw1 = ttg.async_wait %21 {num = 0 : i32}

    // CHECK: amdgpu.async_tdm_store_wait {{.*}} {num = 0
    %w2 = amdgpu.async_tdm_store_wait %3 {num = 0 : i32}
