...
class HIPBackend(BaseBackend):
    ...
    def make_ttgir(mod, metadata, options):
        ...
        if False:
            pm.run(mod)
            with open("mid.mlir", "w") as f:
                print(mod, file=f)
            context = mod.context
            mod = ir.parse_mlir_module("mod.mlir", context)
            mod.context = context
            pm = ir.pass_manager(mod.context)
        ...
        if use_block_pingpong and options.num_stages == 4:
        ...
        #pm.run(mod)
        tname = "/var/lib/jenkins/AMD-triton/python/perf-kernels/study_4-stage/hack_unroll1.ttgir"
        outname = "./tempout.ir"
    ...
    def make_amdgcn(src, metadata, options):
        ...
        if config.amd.dump_amdgcn:
            print("// -----// AMDGCN Dump //----- //")
            print(amdgcn)
        # if amdgcn.find("_attn_fwd")
        # with open("out.amdgcn", "r") as f:
        #     amdgcn = f.read()
