...
private:
  ...
  LogicalResult sliceDot(OpBuilder &builder, Location loc, tt::DotOp op,
                         unsigned numSlices);
  LogicalResult unmergeAndHoistAsyncWait(OpBuilder &builder, Location loc);
...
void Pingponger::determineDotAsyncMemoryOps(
  ...
  for (auto &&asyncCommitGroup : dotAsyncCommitGroups) {
    if (!asyncCommitGroup->hasOneUse())
      return;
    auto yieldOp =
        dyn_cast<scf::YieldOp>(*asyncCommitGroup->getUsers().begin());
    if (!yieldOp)
      return;
    size_t yieldIdx = asyncCommitGroup->use_begin()->getOperandNumber();
    BlockArgument asyncCommitIterArg = forOp.getRegionIterArg(yieldIdx);
    if (!asyncCommitIterArg.hasOneUse())
      return;
    auto asyncWaitOp =
        dyn_cast<ttg::AsyncWaitOp>(*asyncCommitIterArg.getUsers().begin());
    if (!asyncWaitOp)
      return;
...
// Transforms prefetch pipeline GEMM for StreamPipeline to be more ping-pong
// friendly and prevent race condition. This is done by moving the first
// AsyncWait to prologue; hence, moving placement of AsyncWait towards the end
// of the forOp and removing one from epilogue completely. e.g from:
//     async_copy_global_to_local0
//     async_commit_group0
//     for = for (commit_iter = async_commit_group0)
//        async_wait0 = async_wait commit_iter
//        async_copy_global_to_local1
//        async_commit_group1
//        local_load %data, token async_wait0
//        * some compute *
//        yield async_commit_group1
//      async_wait1 = async_wait %for#0
//      local_load token async_wait1
// Into:
//     async_copy_global_to_local0
//     async_commit_group0
//     async_wait0 = async_wait async_commit_group0
//     for = for (wait_iter = async_wait0)
//        async_copy_global_to_local1
//        async_commit_group1
//        local_load %data, token wait_iter
//        * some sliced dots *
//        async_wait1 = async_wait async_commit_group1
//        * one last sliced dot *
//        yield async_wait1
//      local_load token for#0
// Updated IR will now have two s_barrier between AsyncWait and localLoad,
// which guarantees that both wave has completed copying data to local.
LogicalResult Pingponger::unmergeAndHoistAsyncWait(OpBuilder &builder,
                                                   Location loc) {

  auto originalInsertionPoint = builder.getInsertionPoint();
  auto originalInsertionBlock = builder.getInsertionBlock();
  // Check that asyncWait has 2 operand and both are iterArgs.
  // Check that init value of iterArg are asyncCommit from Prologue.
  // If condition holds, then we insert/Hoist AsyncWaitOp to before forOp.
  if (asyncWaitOps[0]->getNumOperands() != 2)
    return failure();

  // Step 0: Match/validate pattern is async prefetch pipeline GEMM. While also
  // searching for asyncCommits in prologue to insert the hoisted asyncWaits
  // onto, and also localLoad inside forOp and epilogue, who's token needs to be
  // updated to asyncWait of previous iter, and asyncWait from forOp result
  // respectively.
  SmallVector<ttg::AsyncCommitGroupOp> prologueAsyncCommitOps;
  SmallVector<std::pair<int64_t, ttg::LocalLoadOp>> tokenToLoopLocalLoad;
  SmallVector<std::tuple<int64_t, ttg::LocalLoadOp, ttg::AsyncWaitOp>>
      tokenToEpiLocalLoadAndWait;
  for (auto asyncToken : asyncWaitOps[0].getAsyncToken()) {
    // Get arg number for asyncToken.
    auto blockArg = dyn_cast<BlockArgument>(asyncToken);
    if (!blockArg)
      return failure();
    int64_t tokenArgIdx = blockArg.getArgNumber() - forOp.getNumInductionVars();
    Value initArg = forOp.getInitArgs()[tokenArgIdx];
    auto prologueAsyncCommitOp =
        dyn_cast<ttg::AsyncCommitGroupOp>(initArg.getDefiningOp());
    if (!prologueAsyncCommitOp)
      return failure();
    prologueAsyncCommitOps.push_back(prologueAsyncCommitOp);

    // Get arg number for LoadOp.
    SmallVector<Value> tokens(prologueAsyncCommitOp.getInputTokens());
    if (tokens.size() != 1)
      return failure();
    auto prologueCopy =
        dyn_cast<ttg::AsyncCopyGlobalToLocalOp>(tokens[0].getDefiningOp());
    if (!prologueCopy)
      return failure();
    std::optional<int64_t> maybeLoadIterArgIdx;
    for (auto [idx, value] : llvm::enumerate(forOp.getInitArgs())) {
      if (value == prologueCopy.getResult()) {
        maybeLoadIterArgIdx = idx;
        break;
      }
    }
    if (!maybeLoadIterArgIdx.has_value())
      return failure();
    int64_t loadIterArgIdx = maybeLoadIterArgIdx.value();

    // Finds and store localLoad in forOp and corresponding asyncToken
    // information.
    Value loadIterArg = forOp.getRegionIterArg(loadIterArgIdx);
    if (!loadIterArg.hasOneUse())
      return failure();
    auto curLocalLoad =
        dyn_cast<ttg::LocalLoadOp>(*loadIterArg.getUsers().begin());
    if (!curLocalLoad)
      return failure();
    tokenToLoopLocalLoad.emplace_back(tokenArgIdx, curLocalLoad);

    // Finds and store localLoad in Epilogue and corresponding asyncToken
    // information.
    Value resultLoad = forOp.getResult(loadIterArgIdx);
    if (!resultLoad.hasOneUse())
      return failure();
    auto epilogueLocalLoad =
        dyn_cast<ttg::LocalLoadOp>(*resultLoad.getUsers().begin());
    if (!epilogueLocalLoad)
      return failure();
    auto epilogueAsyncWaitOp = dyn_cast<ttg::AsyncWaitOp>(
        epilogueLocalLoad.getToken().getDefiningOp());
    if (!epilogueAsyncWaitOp)
      return failure();
    tokenToEpiLocalLoadAndWait.emplace_back(tokenArgIdx, epilogueLocalLoad,
                                            epilogueAsyncWaitOp);
  }

  // Step 1: Hoist first AsyncWait into prologue.
  builder.setInsertionPoint(forOp);
  for (auto prologueAsyncCommitOp : prologueAsyncCommitOps) {
    Value prologueAsyncWait = builder.create<ttg::AsyncWaitOp>(
        loc, ValueRange{prologueAsyncCommitOp}, 0);
    prologueAsyncCommitOp.getResult().replaceUsesWithIf(
        prologueAsyncWait, [&](OpOperand &use) {
          return use.getOwner() != prologueAsyncWait.getDefiningOp();
        });
  }

  // Step 2: Replace old asyncWait token used by localLoad to use
  // iterArg/AsyncWait of previous step.
  for (auto [tokenIdx, curLocalLoad] : tokenToLoopLocalLoad) {
    asyncWaitOps[0].getResult().replaceUsesWithIf(
        forOp.getRegionIterArg(tokenIdx), [&](OpOperand &use) {
          return use.getOwner() == curLocalLoad.getOperation();
        });
  }

  // Since we have hoisted the first asyncWait to prologue, we will need an
  // asyncWait inside the steady state for the AsyncCommits. For now, we will
  // insert them after their AsyncCommit which is sub-optimal but valid. Each
  // PP cluster transform will then select where to best place the async waits.
  // Step 3: Generate new AsyncWait inside the loop on current iteration to be
  // token for next iteration.
  SmallVector<ttg::AsyncWaitOp> newAsyncWaitOps;
  for (ttg::AsyncCommitGroupOp asyncCommitOp : asyncCommitOps) {
    builder.setInsertionPointAfter(asyncCommitOp);
    auto asyncWaitOp =
        builder.create<ttg::AsyncWaitOp>(loc, ValueRange{asyncCommitOp}, 0);
    asyncCommitOp.getResult().replaceUsesWithIf(
        asyncWaitOp.getResult(), [&](OpOperand &use) {
          return use.getOwner() != asyncWaitOp.getOperation();
        });
    newAsyncWaitOps.push_back(asyncWaitOp);
  }

  // Step 4: Transform epilogue from having `async_wait + local_load` to ->
  // local_load, since we have moved the last `async_wait` to inside the loop.
  for (auto [tokenIdx, epilogueLocalLoad, epilogueAsyncWaitOp] :
       tokenToEpiLocalLoadAndWait) {
    epilogueAsyncWaitOp.getResult().replaceUsesWithIf(
        forOp.getResult(tokenIdx), [&](OpOperand &use) {
          return use.getOwner() == epilogueLocalLoad.getOperation();
        });
    if (epilogueAsyncWaitOp->use_empty())
      epilogueAsyncWaitOp->erase();
  }

  // Step 5 clean up old/unused AsyncWait, and register new AsyncWaitOp into
  // AsyncWaitOps member variable.
  if (asyncWaitOps[0]->use_empty())
    asyncWaitOps[0]->erase();
  asyncWaitOps = newAsyncWaitOps;
  builder.setInsertionPoint(originalInsertionBlock, originalInsertionPoint);
  return success();
}
...
void Pingponger::getDotPingponged() {
  ...
  forOp->walk([&](Operation *op) {
    ...
    else if (auto asyncCommitGroupOp = dyn_cast<ttg::AsyncCommitGroupOp>(op))
      asyncCommitOps.push_back(asyncCommitGroupOp);
    else if (auto asyncWaitOp = dyn_cast<ttg::AsyncWaitOp>(op))
      asyncWaitOps.push_back(asyncWaitOp);
  ...
  size_t gLoadCount = useAsyncCopy ? asyncCopyOps.size() : gLoadOps.size();
  if (gLoadCount < 2 || lLoadOps.size() < 2 || dotOps.size() != 1) {
    std::stringstream message;
    message << "Unable to match ping pong scheduling pattern. Details: "
  ...
  if (!useAsyncCopy) {
    ...
    if (failed(unmergeAndHoistAsyncWait(builder, loc))) {
      LDBG("Failed to unmerge/hoist asyncWaitOp required for proper ping pong "
           "scheduling.");
      return;
    }
  }
  ...
  if (numWarps == 4) { // Pingpong between warps from different blocks
    ...
    // TODO: Add asyncOp num_warp=4.
    if (useAsyncCopy) {
      LDBG("Currently AsyncOp do not support numWarps = 4.");
      return;
    }
    ...
    if (lStoreOps.size() != 2 && !useAsyncCopy) {
      std::stringstream message;
      message << "Unable to match ping pong slicing pattern. Details: "
