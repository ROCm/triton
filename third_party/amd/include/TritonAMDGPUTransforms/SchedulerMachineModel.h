...
/*******************************************************************************
  ...
  assumed that DotOps are the most important, that memory ops whcih feed them are
  in a close second, and all other ops are tertiary in importance. Therefore it
  essentially only model ops as being dot, load/stores, nop, and other.
...
/*
  ...
  (3) TopDown Data Example:
    ...
    ready. E.g. currentCycle=16 scheduleOp(load) will record op->children need to
    wait until t=16+updateOpDataReady(load).
