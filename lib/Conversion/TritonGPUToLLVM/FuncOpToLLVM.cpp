...
struct FuncOpConversion : public ConvertOpToLLVMPattern<triton::FuncOp> {
  ...
  static void handleArgPtrDatatype(triton::FuncOp funcOp, LLVM::LLVMFuncOp &llvmFuncOp) {

    // The convertion from triton::PointerType to LLVM::LLVMPointerType losts the 
    // pointee datatype. This function add the pointee datatype to arg attribute.
    ...
    for (unsigned i = 0; i < fty.getNumInputs(); ++i) {
      ...
      if(auto argPtrType = dyn_cast<triton::PointerType>(argType)) {
        ...
        llvmFuncOp.setArgAttr(i, "tt.pointee_type", mlir::TypeAttr::get(argDType));
      }
    }
  }
