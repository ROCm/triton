...
def _fwd_grouped_kernel_stage1(
    ...
    tl.store(token_blk_logits + off_token_blk_logit, attn,
             mask=(offs_h[:, None] < BLOCK_H) & (offs_d[None, :] < BLOCK_DMODEL))
...
def _fwd_grouped_kernel_stage2(
    ...
    ml = tl.load(max_logits + offs_exp, mask=mask_exp)

    #calculate global_max_logit [Q_GRP_SZ]
    max_logit = tl.max(ml, axis=0)

    #load exp_sums[NUM_TOKENS_BLKS, Q_GRP_SZ]
    exp_sum = tl.load(exp_sums + offs_exp, mask=mask_exp)  #TODO add masking
    ...
    inv = tl.reshape(inv, (NUM_TOKEN_BLKS, Q_GRP_SZ, 1))
    ...
    offs_l = offs_tkn[:, None] * stride_token_blk_logits_t + offs_q_grp[None, :]
    mask_l = (offs_tkn[:, None] < NUM_TOKEN_BLKS) & (offs_q_grp[None, :] < Q_GRP_SZ)
    logits = tl.load(token_blk_logits_start_ptr + offs_l, mask=mask_l)
    ...
    off_o = tl.arange(0, Q_GRP_SZ_PADDED)[:, None] + tl.arange(0, BLOCK_DMODEL)[None, :]
    mask_o = offs_q_grp[:, None] < Q_GRP_SZ
...
def _decode_grouped_reduce_fwd(out, token_blk_logits, exp_sums, max_logits, req_to_token, b_req_idx, b_start_loc,
                               ...
                               Q_GRP_SZ):
    batch, num_kv_heads = b_seq_len.shape[0], token_blk_logits.shape[0]
    ...
    _fwd_grouped_kernel_stage2[grid](out, exp_sums, max_logits, token_blk_logits, out.stride(0), out.stride(1),
                                     ...
                                     NUM_TOKEN_BLKS=NUM_TOKEN_BLKS, MAX_NUM_TOKEN_BLKS=NUM_TOKEN_BLKS,
...
#@pytest.mark.parametrize('B, H_Q, H_KV, D', [(1, 2, 1, 16)])
def test_decode_attn_fwd(B, H_Q, H_KV, D):
    ...
    q = torch.tensor([[[
        -1.2148, -0.2076, 0.5088, -0.3228, -0.2861, 0.1880, -0.5200, -0.1700, 1.0332, 0.8286, 1.9727, 0.4199, 1.9316,
        -0.3718, 0.0983, -1.0205
    ],
                       [
                           -2.1914, 0.1006, 0.6709, -2.2168, -0.0374, -2.3164, 1.0762, 0.5073, -0.5654, 0.5112, -0.2058,
                           -0.2881, -0.7549, -0.3745, 1.3682, 0.0166
    ...
    k_buffer = torch.tensor([[[
        0.4797, 0.1538, -0.3738, 0.9897, 0.2727, -0.4761, -0.1324, 0.1216, -0.6768, 0.6147, 0.2069, -2.5430, -0.3311,
        1.3184, 1.9883, 1.1670
    ]]], dtype=dtype, device="cuda")
    v_buffer = torch.tensor([[[
        1.9053, 2.3418, 0.8062, 1.1445, 0.7368, -0.7554, -0.5381, 1.4346, -0.7583, 0.1371, 1.6084, -1.2773, -1.0039,
        0.1445, 0.6479, -1.1885
    ]]], dtype=dtype, device="cuda")
    ...
    assert torch.allclose(triton_output, torch_output, rtol=1e-05, atol=1e-06)
...
def run_benchmark(args):
    config = []
    x_vals_list = [i for i in range(args.batch_sz_start, args.batch_sz_end, args.batch_sz_step)]
    pa_args = {'H_Q': args.num_q_heads, "Q_GRP_SZ": args.q_grp_sz, "D": args.head_dim}
    plot_name = str("pagedattn-performance_" + args.dtype + "_NUM_Q_HEADS" + str(args.num_q_heads) + "_NUM_Q_HEADS" +
                    str(args.num_q_heads) + "_B" + str(args.batch_sz_start) + "-" + str(args.batch_sz_end) + "-" +
                    str(args.batch_sz_step))
    x_names = ['B']
...
def parse_args():
    ...
    parser.add_argument('-bs', "--batch_sz_step", default="1", type=int)
    parser.add_argument('-be', "--batch_sz_end", default="2", type=int)
