...
def get_autotune_configs_persistent():
    if is_rdna():
        return get_rdna_autotune_configs_persistent()
    elif is_cdna():
        return get_cdna_autotune_configs_persistent()
    else:
        raise ValueError("Unknown Device Type")


autotune_configs_persistent, autotune_keys_persistent = get_autotune_configs_persistent()


@triton.autotune(
    configs=autotune_configs_persistent,
    key=autotune_keys_persistent,
    use_cuda_graph=True,
)
@triton.jit
def attn_fwd_persistent(Q, K, V, bias, SM_SCALE: tl.constexpr, L, Out, stride_qz, stride_qh, stride_qm, stride_qk, stride_kz,
             stride_kh, stride_kn, stride_kk, stride_vz, stride_vh, stride_vk, stride_vn, stride_oz, stride_oh,
             stride_om, stride_on, stride_bz, stride_bh, stride_bm, stride_bn, stride_az, stride_ah, cu_seqlens_q,
             cu_seqlens_k, dropout_p, philox_seed, philox_offset_base, encoded_softmax, alibi_slopes, NUM_CU, atomic_counter, CHUNK_SIZE, ZQ: tl.constexpr, HQ: tl.constexpr,
             HK: tl.constexpr, ACTUAL_BLOCK_DMODEL: tl.constexpr, MAX_SEQLENS_Q: tl.constexpr,
             MAX_SEQLENS_K: tl.constexpr, VARLEN: tl.constexpr, IS_CAUSAL: tl.constexpr, BLOCK_M: tl.constexpr,
             BLOCK_DMODEL: tl.constexpr, BLOCK_N: tl.constexpr, PRE_LOAD_V: tl.constexpr, USE_BIAS: tl.constexpr,
    ...
    a workgroup per each Q tile as is done with the standard flash attention (see attn_fwd). 
    ...
    while tile_id < num_tiles_total:
        # tile id basically tells us the Q tile we are handling
        chunk_iter = 0 # go over CHUNK_SIZE contiguous tiles
        ...
        if metadata.persistent:
            ...
            attn_fwd_persistent[grid](q, k, v, metadata.bias, metadata.sm_scale, M, o, *q_strides, *k_strides, *v_strides, *o_strides,
                        *bias_strides, *alibi_strides, metadata.cu_seqlens_q, metadata.cu_seqlens_k,
                        ...
                        encoded_softmax=encoded_softmax, alibi_slopes=metadata.alibi_slopes, NUM_CU=NUM_CU, atomic_counter=atomic_counter, CHUNK_SIZE=CHUNK_SIZE, ZQ=batch, HQ=nheads_q, HK=nheads_k,
...
def test_op_fwd(Z, HQ, HK, N_CTX_Q, N_CTX_K, D_HEAD, causal, use_alibi, layout, persistent, dtype=torch.float16):
    ...
    print(f"ref out: {ref_out.flatten()[:2]}\ntri_out:{tri_out.flatten()[:2]}")
