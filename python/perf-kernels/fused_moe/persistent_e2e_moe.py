...
def e2e_moe_kernel(
    ...
    NUM_WG: tl.constexpr, # number of launched persistent workgroups
):
    """
    ...
    pid_m = tl.program_id(axis=0) # 0, ...., NUM_WG
...
def e2e_moe(a: torch.Tensor, w1: torch.Tensor, w2: torch.Tensor, c: torch.Tensor, metadata: MetaData) -> torch.Tensor:
    ...
    grid = (NUM_WG,) # persistent kernels looping over triton.cdiv(N, META['BLOCK_SIZE_N']) pids
...
def input_helper(M: int, N: int, K: int, top_k: int, E: int, routed_weight: bool, use_fp8_w8a8: bool,
    ...
    config = {'BLOCK_SIZE_M': 32, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 128, 'GROUP_SIZE_M': 1, 'num_warps': 4, 'num_stages': 2, 'waves_per_eu': 1, 'matrix_instr_nonkdim': 16, 'kpack': 2}
...
def get_configs():
    configs = [
        ...
        {"M": 8192, "N": 7168, "K": 4096, "E": 8, "top_k": 2},
...
def model_benchmark_configs(args):
    ...
    moe_configs = []
    M = args.M if args.M else 8192  # check size
    # M, K, N, E, top_k
