...
class AMDRotatingSharedLayout(SharedLayout):
    """
    Represents a rotating swizzled shared memory layout for AMD GPUs.

    Similar to SwizzledSharedLayout, but uses a rotating swizzle pattern across
    blocks to reduce bank conflicts when shared memory writes and reads have
    different access orders. The rotation is computed as:
        phase        = (row / per_phase) % max_phase
        block_no     = (row / (per_phase * max_phase)) % max_phase
        combined     = phase ^ block_no
        out_col      = in_col ^ combined
    ...
    def mangle(self) -> str:
        ...
        return f"ARS_{self.vec}_{self.per_phase}_{self.max_phase}_{stringify(self.order)}_{stringify(self.ctas_per_cga)}_{stringify(self.cta_split_num)}_{stringify(self.cta_order)}_ARS"
